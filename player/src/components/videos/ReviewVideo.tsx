...
type Props = {
  ...
  onEnableOverlays: () => void;
  video: Video;
};

const ReviewVideo = observer(
  ({ hideOverlays, video, onEnableOverlays }: Props) => {
    ...
    const [dimensionsReady, setDimensionsReady] = React.useState<
      boolean | null
    >(null);
    ...
    const [controlsHovered, setControlsHovered] = React.useState<
      boolean | null
    >(null);

    // Holds the mounted of instance of the TLDraw app. This is passed down to
    // the drawing controls widget.
    const [tlDrawInstance, setTlDrawInstance] =
      React.useState<TldrawApp | null>(null);
      ...
      >
          ...
          {({ width, height, scale }) => {
            ...
            return (
              <div style={dimensionsStyle} className="relative">
                <div className="absolute left-0 bottom-0 z-40 text-white flex items-center gap-4">
                  {videoNotReady === true && (
                    <button
        ...
        {hideOverlays === false &&
          ...
          selectedBookmark !== undefined && (
            <div
              className="z-30 absolute inset-0 pointer-events-none flex items-end justify-center"
              style={{ bottom: "150px" }}
            >
