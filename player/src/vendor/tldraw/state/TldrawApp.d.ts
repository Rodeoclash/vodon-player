...
export declare class TldrawApp extends StateManager<TDSnapshot> {
   ...
   */
  getImage: (
    format?: Exclude<TDExportType, TDExportType.JSON>,
    opts?: Partial<{
      ids: string[];
      scale: number;
      quality: number;
   ...
   */
  copyImage: (
    format?: TDExportType.PNG | TDExportType.SVG,
    opts?: Partial<{
      ids: string[];
      scale: number;
      quality: number;
  ...
  startSession: <T extends SessionType>(
    type: T,
    ...args: import("../types").ExceptFirst<
      ConstructorParameters<import("./sessions").SessionOfType<T>>
    >
  ) => this;
  /**
   * updateSession.
  ...
  getImageOrVideoShapeAtPoint(
    ...
    size: number[],
    assetId: string
  ): import("../types").ImageShape | import("../types").VideoShape;
  /**
